...
def check_word_exists(word: str, app_key: str) -> bool:
    ...
    if response.status_code == 200:
        data = response.json()
        return isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict) and 'meta' in data[0]
    elif response.status_code == 404:
        return False
    else:
        response.raise_for_status()

def get_five_letter_words() -> str:
    url = "https://api.datamuse.com/words"
    params = {
        "sp": "?????",  # 5-letter words
        "max": 10,      # you can adjust the number of results as needed
...
def valid_input(input: str, app_key: str) -> bool:
    if len(input) != 5:
        return False
    for char in input:
        if ord(char) not in range(97, 123):  # ASCII range for lowercase letters a-z
            return False
    if not check_word_exists(input, app_key):
        print("Could not find this word!")
        return False
...
def game(game_dic: dict, limit: int, app_key: str) -> dict:
    guessd_word = input(f"You have {limit - game_dic['attempt']} attempts left.\n\n")
    if not valid_input(guessd_word.lower(), app_key):
        game_dic["status"] = False
        game_dic["message"] = "\nNot a valid word, guess again."
        return game_dic
    if guessd_word.lower() == game_dic["actual_word"]:
        game_dic["status"] = True
        game_dic["attempt"] += 1
        game_dic["message"] = f"\nCongratulations! You have completed the challenge with {game_dic['attempt']} attempts!!!"
        return game_dic
    else:
        for char in guessd_word.lower():
            if char not in game_dic["actual_word"]:
                if char not in game_dic["letters_out"]:
                    game_dic["letters_out"].append(char)
            elif char not in game_dic["letters_in"]:
                game_dic["letters_in"].append(char)
        game_dic["attempt"] += 1
...
def word_guess(app_key: str, limit=6):
    word = get_five_letter_words()
    game_dic = {
        "status": False,
        "attempt": 0,
        "message": "You have guessed wrong, try again.",
        "letters_in": [],
        "letters_out": [],
        "actual_word": word
    }
    check = input("\nWould you like to play a game of word guess?\n\n")
    if check.lower() == "yes":
        print(f"\nGuess a 5 letter word, you get {limit} attempts.\n")
        while not game_dic["status"] and game_dic["attempt"] < limit:
            game_dic = game(game_dic, limit, app_key)
            print(game_dic["message"])
        if game_dic["status"]:
            print("\nThanks for playing, see you again!")
        else:
            print(f"\nYou have used up your chances. The correct word was '{word}'.")
            re_check = input("\nWould you like to play again?\n")
            if re_check.lower() == "yes":
                word_guess(app_key, limit)
            else:
                print("\nThanks for playing, see you again!")
    else:
        print("Maybe next time.")
